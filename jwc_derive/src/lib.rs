...
/// Derive `JwcSerializable` for a struct with named fields.
///
/// # Panics
/// On tuple structs: every field must be named.
...
/// Derive `JwcDeserializable` for a struct with named fields.
///
/// # Panics
/// On tuple structs: every field must be named.
            ...
            Fields::Named(fields) => {
                    ...
                    quote! {
                        #name: {
                            if let Some(entry) = map.remove(#name_str) {
                                ...
                                return Err(jwc::Error::type_mismatch(format!("Missing field: {}", #name_str)));
                            }
                        },
                    }
                });
                ...
                quote! {
                     if let jwc::Value::Object(members) = value {
                        ...
                        Err(jwc::Error::type_mismatch("Expected Object for struct"))
                    }
                }
            }
