...
#[test]
fn test_lib_to_string_pretty_rejects_odd_indent() {
    let n = Node::new(Value::from(true));
    let err = jwc::to_string_pretty(&n, Some("xyz")).unwrap_err();
    assert!(matches!(err.kind, jwc::ErrorKind::Format(_)));
    assert_eq!(jwc::to_string_pretty(&n, Some("  ")).unwrap(), "true");
}
