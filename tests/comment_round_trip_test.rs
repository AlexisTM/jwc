...
fn test_node_comments() {
    ...
    node.comment(Trivia::line(" leading "));
    node.comment(Trivia::block(" block "));
    ...
    node.comment(Trivia::line(" trailing "));
...
fn test_object_entry_comments() {
    ...
    val_node.comment(Trivia::line(" value comment "));

    let mut entry = ObjectEntry::new("key".to_string(), val_node);
    entry.key_comment(Trivia::line(" key comment "));
    entry.key_comment(Trivia::block(" key trailing "));
...
fn test_number_lexeme_round_trip_and_parse_on_demand() {
    ...
    assert!(
        serialized
            .contains("[1.23456789e+308,5e-324,-0.0,0.0000000000000000000000000000000000000001]"),
