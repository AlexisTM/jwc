...
#[test]
fn test_serializer_trailing_comma_object_entry() {
    let mut entry = ObjectEntry::new("a".to_string(), Node::new(Value::from(1)));
    entry.value.comma = true;
    let obj = Value::Object(vec![entry]);
    let n = Node::new(obj);
...
fn test_parser_more_errors_specific() {
    ...
    assert!(jwc::from_str("{\"a\" \u{00A0} : 1}").is_err()); // Only JSON whitespace is whitespace
...
fn test_manual_object_no_trailing_comma() {
    ...
    // Ensure trailing_comma is false
    if let Value::Object(ref mut members) = obj {
        members[0].value.comma = false;
        members[1].value.comma = false;
    }
    let n = Node::new(obj);
    let s = jwc::to_string(&n).unwrap();
    assert_eq!(s, "{\"a\":1,\"b\":2}");
