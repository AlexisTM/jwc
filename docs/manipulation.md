...
3. Serialize with `to_string_preserving(&node, &source, None)` to touch only
   the edited containers, or `to_string_pretty` to re-lay out everything.

Patching `node.value` in place keeps the comments and layout of the rest of the
document; building a fresh `Node::new(value)` would drop the root comments.
