...
pub trait JwcDeserializable {
    ...
    ///
    /// # Errors
    /// `ErrorKind::Type` when the value has the wrong shape for `Self`.
...
impl JwcDeserializable for bool {
    fn from_jwc(value: Value) -> Result<Self> {
        match value {
            Value::Bool(b) => Ok(b),
            _ => Err(Error::type_mismatch("Expected Bool")),
        }
    }
}
            ...
            impl JwcDeserializable for $t {
                fn from_jwc(value: Value) -> Result<Self> {
                    match value {
                        ...
                        _ => Err(Error::type_mismatch(format!("Expected Number for {}", stringify!($t)))),
                    }
                }
            }
        )*
...
impl JwcDeserializable for () {
    fn from_jwc(value: Value) -> Result<Self> {
        match value {
            Value::Null => Ok(()),
            _ => Err(Error::type_mismatch("Expected Null for unit")),
        }
    }
}
...
impl JwcDeserializable for String {
    fn from_jwc(value: Value) -> Result<Self> {
        match value {
            Value::String(s) => Ok(s),
            _ => Err(Error::type_mismatch("Expected String")),
        }
    }
}
...
impl<T: JwcDeserializable> JwcDeserializable for Vec<T> {
    fn from_jwc(value: Value) -> Result<Self> {
        match value {
            ...
            _ => Err(Error::type_mismatch("Expected Array")),
        }
    }
}

impl<T: JwcSerializable, S: BuildHasher> JwcSerializable for HashMap<String, T, S> {
    fn to_jwc(&self) -> Value {
        // HashMap has no order; members come out in iteration order.
...
impl<T: JwcDeserializable, S: BuildHasher + Default> JwcDeserializable for HashMap<String, T, S> {
    fn from_jwc(value: Value) -> Result<Self> {
        match value {
            Value::Object(members) => {
            ...
            _ => Err(Error::type_mismatch("Expected Object")),
        }
    }
}
